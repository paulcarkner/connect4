...
class App extends React.Component<{}> {
  ...
  componentDidUpdate(
    ...
    if (this.state.winner !== "") { //show modal on winner
  ...
  handleClick = (e: React.MouseEvent): void => {
    ...
    for (let x = 5; x >= 0; x--) { //iterates from bottom to top of column
      if (newBoard[columnIndex][x] === null) { //finds first empty cell
  ...
  checkWinner = () => {
    let lastToken: string = null; //traking which colour the current string of tokens is
    ...
    for (let x = 0; x < 7; x++) {
      for (let y = 0; y < 6; y++) {
        ...
        if (tokenCount === 4 && lastToken !== null) { //if current run is 4...
    ...
    for (let x = 3; x < 7; x++) { //starting cell must be at least 3 away from left and bottom
      for (let y = 0; y < 3; y++) {
        if (
    ...
    for (let x = 0; x < 4; x++) { //starting cell must be at least 3 away from right and bottom
      for (let y = 0; y < 3; y++) {
        if (
          this.state.board[x][y] === this.state.board[x + 1][y + 1] &&
          this.state.board[x][y] === this.state.board[x + 2][y + 2] &&
